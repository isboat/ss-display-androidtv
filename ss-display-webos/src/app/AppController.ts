...
export class AppController {
  private readonly api: ApiClient; private readonly realtime: SignalRService; private polling = false; private contentRetryTimer = 0;
  ...
  private async activate(): Promise<void> {
    try {
      ...
      while (this.polling && Date.now() < expiresAt) { await this.delay((code.interval ?? 5) * 1000); try { this.storage.tokens = await this.api.request<Tokens>(config.deviceTokenRequestUrl, { method: 'POST', body: JSON.stringify({ clientId: code.clientId, clientSecret: 'string', deviceCode: code.deviceCode, grantType: 'urn:ietf:params:oauth:grant-type:access_token' }) }); this.polling = false; return this.loadDisplay(); } catch (error) { if (!(error instanceof ApiError) || error.status !== 428) throw error; } }
      throw new Error('The activation code expired.');
    } catch (error) { this.fail(error); }
  }
  private async loadDisplay(): Promise<void> {
    window.clearTimeout(this.contentRetryTimer);
    try {
      const config = this.requiredConfig();
      const device = await this.api.request<DeviceInfo>(config.deviceInfoUrl); this.storage.device = device;
      const content = await this.api.request<ContentData>(config.contentDataUrl); this.storage.content = content;
      this.router.show('content', { content });
      void this.realtime.connect(config, device).catch(() => undefined);
    } catch (error) {
      if (this.storage.content) {
        this.router.show('content', { content: this.storage.content });
        this.showNotice('Offline · showing last known content', false);
        this.contentRetryTimer = window.setTimeout(() => { void this.loadDisplay(); }, 30_000);
