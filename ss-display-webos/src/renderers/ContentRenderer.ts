...
export class ContentRenderer {
  ...
  public render(content: ContentData, host: HTMLElement): void {
    this.clearClock(); host.replaceChildren(); const key = content.layout?.templateKey?.toLowerCase() ?? '';
    ...
    if (key.includes('text')) return this.renderText(content.textEditorData ?? '', host);
  ...
  public stop(): void { this.clearClock(); this.player.stop(); }
  ...
  private renderMenu(content: ContentData, host: HTMLElement, overlay = false): void {
    ...
    const list = document.createElement('div'); list.className = 'menu-grid'; for (const item of content.menu?.menuItems ?? []) list.append(this.menuItem(item, content.menu?.currency ?? '')); section.append(list); host.append(section);
  }
  ...
  private renderClock(content: ContentData, host: HTMLElement): void { const clock = document.createElement('time'); clock.className = 'clock'; host.append(clock); const update = () => { clock.textContent = new Intl.DateTimeFormat(undefined, { dateStyle: 'full', timeStyle: 'medium' }).format(new Date()); }; update(); this.clockTimer = window.setInterval(update, 1000); void content; }
