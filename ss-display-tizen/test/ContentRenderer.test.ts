...
import type { AvPlayAdapter } from '../src/media/AvPlayAdapter';

describe('ContentRenderer', () => {
  it('renders the backend media, text, menu, and discount field names', () => {
    const player = { play: vi.fn(), stop: vi.fn() } as unknown as AvPlayAdapter;
  ...
  it('clears the clock timer when rendering is stopped', () => {
    ...
    const renderer = new ContentRenderer({ stop: vi.fn() } as unknown as AvPlayAdapter); const host = document.createElement('main');
